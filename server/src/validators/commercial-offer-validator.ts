  ...
  async function createCommercialOfferValidator(req: any, res: any, next: any) {
    ...
    const schema = Joi.object({
      serviceRequestId: Joi.string()
        .uuid()
        .valid(...serviceRequestIds),
      clientId: Joi.string()
        .uuid()
        .required()
        .valid(...clientIds),
      wayToPay: Joi.number().valid(WayToPay.CASH, WayToPay.TRANSFER, WayToPay.MIXED).required(),
    ...
    if (req.body.services) {
      for (let i = 0; i < req.body.services.length; i++) {
        const { errorMessages: serviceErrorMessages, value: serviceValue } = validate(
          serviceSchema,
          req.body.services[i],
          `services[${i}].`
        );
  ...
  async function updateCommercialOfferValidator(req: any, res: any, next: any) {
    ...
    const schema = Joi.object({
      ...
      clientId: Joi.string()
        .uuid()
        .valid(...clientIds),
      wayToPay: Joi.number().valid(WayToPay.CASH, WayToPay.TRANSFER, WayToPay.MIXED),
      minimumRequirements: Joi.string(),
      services: Joi.array().items(),
    });

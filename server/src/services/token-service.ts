...
export const useTokenService = () => {
  ...
  async function generateAuthTokens(user: any): Promise<GenerateAuthTokensResponse> {
    ...
    const accessToken: string = generateToken(user.uuid, accessTokenExpires, TokenType.ACCESS);
    const refreshTokenExpires = moment().add(config.jwt.refreshExpirationDays, 'days');
    const refreshToken = generateToken(user.uuid, refreshTokenExpires, TokenType.REFRESH);
    const authTokens = [];
    authTokens.push({
      token: accessToken,
      userId: user.id,

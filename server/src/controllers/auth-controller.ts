...
export const useAuthController = () => {
  ...
  async function login(req: any, res: any) {
    try {
      ...
      const user = await authService.loginWithEmailPassword(email.toLowerCase(), password);
  ...
  async function refreshTokens(req: any, res: any) {
    try {
      ...
      const refreshTokenDoc = await tokenService.verifyToken(token, TokenType.REFRESH);
  ...
  async function removeTokens(req: any, res: any) {
    try {
      ...
      const accessTokenDoc = await tokenService.verifyToken(accessToken, TokenType.ACCESS);

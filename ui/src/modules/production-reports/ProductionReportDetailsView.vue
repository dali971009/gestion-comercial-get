  ...
  <page-view title="Añadir cliente" :use-card="false">
    ...
    <v-form>
        ...
        <v-row>
          <responsive-form-field>
            <custom-input label="Área o brigada" :required="true">
              <v-text-field
                  placeholder="Inserte el área o brigada"
                  v-model="productionReport.area"
                  hide-details
              />
            </custom-input>
          </responsive-form-field>
          <responsive-form-field>
            <custom-input label="Periodo" :required="true">
              <v-text-field
                  ...
                  v-model="productionReport.period"
                  hide-details
              />
            </custom-input>
          </responsive-form-field>
          <responsive-form-field>
            <custom-input label="Plan de ingresos" :required="true">
              <v-text-field placeholder="Inserte el plan de ingresos" v-model="productionReport.incomePlan" type="number" hide-details />
            </custom-input>
          </responsive-form-field>
        </v-row>
      </v-sheet>
      <v-sheet class="v-row ma-2 pa-4" border>
        ...
        <div v-if="productionReport.providedServices.length === 0">
          No hay datos de servicios brindados.
        </div>
        <div v-else>
          <v-sheet class="v-row ma-2 pa-4" v-for="(service, index) in productionReport.providedServices" :key="'service_' + index" border>
            <service-provided-form v-model="productionReport.providedServices[index]"
...
const totalAmount = computed(() => {
  ...
  for (const providedService of productionReport.value.providedServices) {
    if (providedService.service !== undefined && providedService.service.price !== undefined && providedService.quantity !== undefined) {

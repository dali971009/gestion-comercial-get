<template>
  <easy-data-table :headers="headers" :items="services" table-class-name="data-table" border-cell hide-footer>
    ...
    <template #item-invoiceFrequency="item">
      <span v-if="typeof item.invoiceFrequency !== 'undefined'">{{ getFrequencyLabel(item.invoiceFrequency) }}</span>
      <span v-else>-</span>
    </template>
  </easy-data-table>
</template>
